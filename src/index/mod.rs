...
unsafe fn make_routine() -> *mut pg_sys::IndexAmRoutine {
    ...
    // Capabilities — derived from turbovec's feature set:
    //   * orderbyop yes  (we satisfy ORDER BY x <#> q LIMIT k)
    //   * canorder no    (no total order, only nearest-to-query)
    //   * canbackward no (the heap order has no meaning here)
    //   * canunique no
    //   * canmulticol no (one tvector column per index)
    //   * optionalkey yes (no equality predicate required)
    //   * searcharray no
    //   * searchnulls no
    //   * storage yes    (we manage our own storage via am_storage)
    //   * clusterable no
    //   * predlocks no
    //   * canparallel no  (Phase 5)
    //   * canbuildparallel no
    //   * caninclude no
    //   * usemaintenancework_mem yes (build pulls full corpus)
    //   * summarizing no
...
fn turbovec_index_handler(_fcinfo: pg_sys::FunctionCallInfo) -> pg_sys::Datum {
    unsafe {
        // Returning a non-null IndexAmRoutine* coerced to Datum.
