#!/usr/bin/env python3
"""Load SIFT-1M + GIST-1M into dual-column corpus tables.
Each table gets emb public.vector(dim) (pgvector: HNSW/DiskANN/vchord)
and embt turbovec.vector (pg_turbovec) -- identical embeddings, so all
four engines index the SAME data. Ground truth comes from the HDF5
test/neighbors datasets at query time (g0_driver.load_gt)."""
import sys, time
import numpy as np
import h5py
import psycopg2
