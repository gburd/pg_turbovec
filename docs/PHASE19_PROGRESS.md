...
src/tvector.rs payload (small smoke writes succeeded; the full file
...
1. impl PostgresType for Tvector {} -- marker trait
   (pgrx::datum::PostgresType).
2. impl IntoDatum for Tvector -- into_datum(self) returns
   ...
   the SQL type MUST be named exactly Tvector (case-insensitive in
   unquoted SQL -- matches the existing CREATE TYPE Tvector; in the
   generated pgrx schema).
3. impl FromDatum for Tvector --
...
4. unsafe impl UnboxDatum for Tvector with type As<'dat> = Self where
   Self: 'dat -- delegate to FromDatum.
5. unsafe impl<'fcx> ArgAbi<'fcx> for Tvector -- call
   arg.unbox_arg_using_from_datum().unwrap_or_else(|| panic!(...)).
6. unsafe impl BoxRet for Tvector -- match IntoDatum::into_datum,
   return null or fcinfo.return_raw_datum(datum).
7. unsafe impl SqlTranslatable for Tvector --
   argument_sql() = SqlMapping::As("Tvector".into()),
   return_sql() = Returns::One(SqlMapping::As("Tvector".into())).
...
pgrx-macros-0.17.0/src/lib.rs:400 for Complex:
    ...
    extension_sql!(
        ...
        creates = [Type(Tvector)],
    );

    // ...pg_extern fns tvector_in/_out/_recv/_send...

    extension_sql!(
        r#"CREATE TYPE Tvector (
            INPUT          = tvector_in,
            OUTPUT         = tvector_out,
...
requires = [Tvector, ...] keep working because the shell entity is
registered under the Rust path Tvector via creates = [Type(Tvector)].
...
Easiest pgrx-side spelling for send:
    ...
    fn tvector_send(v: Tvector) -> Vec<u8> {
        let n = v.dim();
...
## Files that touch Tvector::data or Tvector::from_vec directly

(grep over the clean 65a3f15 tree)

  src/tvector.rs                         -- full rewrite (the work)
  src/cast.rs                            -- tvector_to_array reads v.data;
                                            switch to v.as_slice().to_vec()
  src/distance.rs                        -- Tvector::from_vec only
  src/aggregate.rs                       -- Tvector::from_vec only
  src/normalize.rs                       -- Tvector::from_vec only
  src/extras.rs                          -- Tvector::from_vec only
  src/knn.rs                             -- query.as_slice() only
  src/index/{build,insert,scan}.rs       -- pgrx::FromDatum::from_datum
                                            only; works with new impl

Net: the only required ripple is src/cast.rs::tvector_to_array's
...
1. TvectorAccum is also #[derive(PostgresType)] (CBOR). It is
...
2. Do NOT impl Drop for Tvector. The wrapped pointer is palloc'd in a
   Postgres memory context; freeing on Rust drop will corrupt the
   backend.
3. Drop the derive(Clone, PartialEq) on Tvector. A grep
   (\.clone\(\)|Tvector\b.*==|PartialEq) shows zero call sites outside
   the derive itself.
...
6. The forced-index-scan test (index_am_forced_index_scan) was un-
   ...
   tvector handling does not regress it -- amrescan copies ScanKey
   datums and invokes the new FromDatum impl on the order-by argument.

## Suggested order of operations next pass

1. Replace src/tvector.rs with the new layout + traits + in/out/recv/send.
2. Patch src/cast.rs::tvector_to_array (v.data -> v.as_slice().to_vec()).
...
5. Add a binary_compatible_with_pgvector_wire_format #[pg_test] that
   round-trips '[1,2,3]'::tvector through tvector_send / tvector_recv
