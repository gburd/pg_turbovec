...
| **Warm scan p50 (1 M × 1536-d, dbpedia-1M ada-002)** | 61 ms (ef=40) / 115 ms (ef=200) | **post-Phase-R-3 (v1.5.0): mmap path landed; arnold re-bench queued** — prior v1.4.0 figure was 90 ms | ⚠️ **closing the gap to HNSW ef=40**. Phase R-3 (v1.5.0, Phase R-3 commit on `phase-r3-mmap-static-regions`) replaces the buffer-manager reads of the deterministic static regions (blocked codes + rotation matrix) with `mmap(MAP_PRIVATE)` of the relfile, so warm scans bypass `ReadBufferExtended` / shared_buffers churn for those bytes. Re-bench on arnold queued for the next bench window; expected p50 drop from 90 ms toward ~60 ms (HNSW ef=40 parity). v1.1.0 was 70 ms; v1.3.0 (Phase P) was 87 ms; v1.4.0 (Phase R-2) was 90 ms. See `docs/RECALL.md` § 2.5 / 2.6. |
