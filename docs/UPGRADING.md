...
| 1.27.3 | 1.27.4+ | _none_ (but REINDEX a corrupt index) | v1.27.4 backports the v1.28.2 duplicate-id detection fix onto the 1.27.x (pgrx 0.17.0) line, so it builds on packaged nixpkgs without the pgrx 0.19.1 / Rust 1.96 jump 1.28.x needs. A duplicate-id corrupt `.tvim` (which `pg_resetwal`/unclean shutdown can leave) now ERRORs at read/open with `HINT: REINDEX INDEX <name>;` (flat + graph kinds; IVF soft-assign dups exempt) instead of silently mis-serving reads while failing every write. No wire/SQL change; `ALTER EXTENSION pg_turbovec UPDATE TO '1.27.4';` suffices. An ALREADY-corrupt index needs `REINDEX INDEX <name>;`. |
