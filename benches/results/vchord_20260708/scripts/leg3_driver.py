...
# ---------- VectorChord (vchordrq: IVF + RaBitQ) ----------
# Appended to leg3_driver.py. Mirrors run_hnsw/run_diskann: build a
# few index variants, sweep the query-time probes/rerank knobs, record
# recall@10 / p50 / qps against the SAME shared heap + published GT.
#
# VectorChord's vchordrq index takes a build-time config (number of
# IVF lists via internal k-means) in a WITH (options = $$...$$) TOML
# block, and two query-time GUCs: vchordrq.probes (cells probed) and
# vchordrq.epsilon (rerank breadth; higher = more exact recheck).
def run_vchord(corpus):
    tab = TABS[corpus]
    test, gt = load_gt(corpus)
    print(f"[vchord {corpus}] q={len(test)} load={loadavg()} memGB={mem_avail_gb():.0f}", flush=True)
    rows = []
    # build variants: lists chosen ~ sqrt(n)*k, matching the turbovec
    # and HNSW build tiers so the comparison is at comparable coarse
    # granularity. RaBitQ residual quantization is on by default.
    if corpus == "sift1m":
        lists_variants = [("L1000", 1000), ("L4000", 4000)]
    elif corpus == "gist1m":
        lists_variants = [("L1000", 1000), ("L4000", 4000)]
    else:  # syn5m
        lists_variants = [("L4000", 4000), ("L8944", 8944)]

    for label, nlists in lists_variants:
        drop_all_on(tab)
        idx = f"{tab}_vchord_{label}"
        # residual_quantization + spherical_centroids are the RaBitQ
        # defaults recommended for L2/cosine corpora respectively;
        # keep L2 to match HNSW/DiskANN/turbovec (all use vector_l2_ops
        # / <-> here).
        options = (
            "[build.internal]\n"
            f"lists = [{nlists}]\n"
        ...
        create = (
            f"CREATE INDEX {idx} ON {tab} USING vchordrq (emb vector_l2_ops) "
            f"WITH (options = $$\n{options}$$)"
        )
        ...
        try:
            ...
            dump(f"vchord_{corpus}", rows)
            continue
        confirm_one_index(tab)
        print(f"[vchord] {label} built build_s={b['build_s']} idx_MB={b['idx_bytes']/1e6:.0f}", flush=True)
        # query sweep: probes x epsilon (rerank breadth). probes trades
        # recall for latency (like turbovec.probes / hnsw.ef_search);
        # epsilon widens the RaBitQ rerank pool.
        for probes in [10, 30, 100, 300]:
            for eps in [1.0, 1.9, 3.0]:
                ...
                try:
                    ...
                    print(f"  probes={probes} eps={eps}: FAILED {e}", flush=True)
                    continue
                row = {"engine": "vchord", "variant": label, "lists": nlists,
                       "probes": probes, "epsilon": eps, **b, **r, "load": loadavg()}
                rows.append(row)
                print(f"  probes={probes:3d} eps={eps}: R@10={r['recall']} p50={r['p50']}ms qps1={r['qps_1conn']}", flush=True)
                dump(f"vchord_{corpus}", rows)
    return rows
