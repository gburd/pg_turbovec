...
│   ├── vec.rs            # the SQL type, text I/O
│   ├── distance.rs           # operators + named distance functions
│   ├── aggregate.rs          # avg(vector), sum(vector)
│   ├── cast.rs               # array <-> vector casts
...
- [x] `vector` type with text I/O (CBOR varlena)
- [x] All distance operators and named functions
- [x] Element-wise arithmetic
- [x] `avg(vector)` / `sum(vector)` aggregates with `f64` accumulator
- [x] Casts from `real[]`, `double precision[]`, `integer[]` and to `real[]`
- [x] `vec_normalize`, `turbovec_self_score`
...
- [x] `subvector(vector, integer, integer)`.
- [x] `vec_to_jsonb` / `jsonb_to_vec` and casts.
- [x] `vec_check_dim`, `vec_zeros`, `vec_to_text`.
