...
| 1.28.4 | 1.29.0+ | _none_ | v1.29.0 adds partitioned-scale support (docs/PARTITIONED_SCALE.md + `turbovec.partition_summary` / `refresh_partition_summary` / `nearest_partitions` for partition pruning toward 1T+ vectors) and the 1-bit sign-BQ foundation (`WITH (bit_width = 1)` reloption accepted; the build path is not yet implemented and ERRORs clearly). Purely additive SQL surface, no wire change (stays v7), **no REINDEX**. `ALTER EXTENSION pg_turbovec UPDATE TO '1.29.0';` suffices. |
